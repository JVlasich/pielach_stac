...
## Quick-Reference Summary - Raster

| Extension               | Fields Added                                                                                                                                                                                                        | Use When                                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `raster`                | `raster:sampling`, `raster:bits_per_sample`, `raster:spatial_resolution`, `raster:scale`, `raster:offset`, `raster:histogram` (+ core band fields: `data_type`, `nodata`, `unit`, `statistics`)                     | Always — core raster metadata (GSD, data type, nodata, scaling, histogram)                                           |
| `projection`            | `proj:epsg`, `proj:wkt2`, `proj:projjson`, `proj:bbox`, `proj:geometry`, `proj:centroid`, `proj:shape`, `proj:transform`                                                                                            | Always — CRS, pixel grid and spatial footprint                                                                       |
| `eo`                    | `eo:bands` (`name`, `common_name`, `center_wavelength`, `full_width_half_max`), `eo:cloud_cover`, `eo:snow_cover`                                                                                                   | For **orthophoto mosaics** — describing RGB / NIR / multispectral bands (omit for DTM/DSM)                           |
| `accuracy`              | `accuracy:geometric_x_bias`, `accuracy:geometric_y_bias`, `accuracy:geometric_x_stddev`, `accuracy:geometric_y_stddev`, `accuracy:geometric_rmse`, `accuracy:measurement_absolute`, `accuracy:measurement_relative` | For **DTMs/DSMs** and georeferenced orthos — horizontal/vertical RMSE, bias, measurement uncertainty                 |
| `classification`        | `classification:classes`, `classification:bitfields`                                                                                                                                                                | When a band encodes categorical values (e.g. DSM ground/building/veg masks, ortho QA flags)                          |
| `rendering` (`renders`) | `renders` object with `assets`, `title`, `rescale`, `colormap_name`, `resampling`, `nodata`, `bands`, `expression`                                                                                                  | When you want default visualizations baked in (DTM/DSM hypsometric tints or hillshade, ortho RGB stretch)            |
| `file`                  | `file:size`, `file:checksum`, `file:header_size`, `file:byte_order`                                                                                                                                                 | When consumers need to validate downloads                                                                            |
| `timestamps`            | `published`, `expires`, `unpublished`                                                                                                                                                                               | When tiles have lifecycle or are published in batches                                                                |
| `version`               | `version`, `deprecated` + version links                                                                                                                                                                             | When tiles are reprocessed and versioned over time                                                                   |
| `processing`            | `processing:lineage`, `processing:level`, `processing:software`, `processing:facility`, `processing:expression`                                                                                                     | When documenting the photogrammetry/LiDAR pipeline (DSM-from-imagery, ground filtering for DTM, ortho-rectification) |
| `storage`      | `storage:platform`, `storage:region`, `storage:requester_pays`, `storage:tier`                                                                                            | For cloud-hosted catalogs with access cost implications                                                   |
